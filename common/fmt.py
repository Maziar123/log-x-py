"""Type-specific formatters for the LogXPy ecosystem.

This module provides formatters for complex data types like DataFrames,
Tensors, and Images.
"""
...
from .base import get_module, truncate
...
class TensorFormatter:
    """Formatter for tensor-like objects (numpy, torch, tensorflow)."""
...
class ImageFormatter:
    """Formatter for PIL/Pillow Images."""
...
class PlotFormatter:
    """Formatter for matplotlib figures."""

    def supports(self, obj: Any) -> bool:
        plt = get_module("matplotlib.pyplot")
        return plt is not None and hasattr(obj, "figure")

    def format(self, obj: Any, **_: Any) -> dict[str, Any]:
        return {
            "_type": "Figure",
            "size": list(obj.get_size()),
            "dpi": obj.get_dpi(),
            "title": obj._suptitle.get_text() if hasattr(obj, "_suptitle") else None,
        }


# Combined formatter list
_FMT: list[Formatter] = [
    DFFormatter(),
    TensorFormatter(),
    ImageFormatter(),
...
def format_value(obj: Any, **opts: Any) -> Any:
    ...
    Args:
        ...
        **opts: Additional options for the formatter.
