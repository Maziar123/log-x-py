"""Shared Sqid (Sequential Quick ID) generation and parsing.
...
- logxpy._sqid (generation)
- logxy_log_parser.sqid (parsing)
"""
...
# Alias for backwards compatibility with logxpy._sqid
def _encode(n: int) -> str:
    """Encode int to base62 (alias for _encode_base62)."""
    return _encode_base62(n)


# ============================================================================
# SqidGenerator - Thread-safe Sqid generation
# ============================================================================
...
class SqidGenerator:
    ...
    def __init__(self, prefix_len: int = 2):
        self._prefix = _encode_base62(os.getpid())[-prefix_len:].rjust(prefix_len, "0")
        self._counter = count(1)
        self._lock = Lock()

    def generate(self) -> str:
        """Generate a new root-level Sqid."""
        with self._lock:
            return f"{self._prefix}.{_encode_base62(next(self._counter))}"

    def child(self, parent: str, index: int) -> str:
        """Generate a child Sqid for a parent Sqid."""
        return f"{parent}.{_encode_base62(index)}"
...
def _get_gen() -> SqidGenerator:
    """Get or create the global SqidGenerator instance."""
    global _gen
    if _gen is None:
        _gen = SqidGenerator()
    return _gen
...
def generate_task_id(use_sqid: bool = True) -> str:
    ...
    Args:
        use_sqid: If True, generate Sqid; otherwise generate UUID4.

    Returns:
        A task ID string (Sqid or UUID4 format)
    """
    if use_sqid:
        return sqid()
...
# ============================================================================
# SqidParser - Parse and analyze Sqid strings
# ============================================================================
...
class SqidParser:
    ...
    # Base62 alphabet for encoding/decoding (same as module-level)
    _ALPHABET = _ALPHABET
    _BASE = _BASE
    ...
    def parse(cls, sqid: str) -> SqidInfo:
        ...
        if counter_str:
            counter = _decode_base62(counter_str)
        else:
...
__all__ = [
    ...
    # Parsing
    "SqidParser", "SqidInfo", "parse_sqid", "sqid_parent", "sqid_depth", "sqid_root",
    # Encoding
    "_encode_base62", "_decode_base62", "_encode",  # _encode for backwards compat
]
