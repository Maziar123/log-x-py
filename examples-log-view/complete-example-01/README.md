...
## 🚀 Quick Minimal Demo (NEW!)

Ultra-compact example showing **7 categories of logxpy features** in just **3 files, ~180 lines total**:

| File | Lines | Purpose | Categories |
|------|-------|---------|------------|
| `minimal_color_demo.py` | ~70 | Create log | System, Memory, Actions, Data Types, Colors, Errors, Checkpoints |
| `minimal_parser.py` | ~70 | Parse log | Shows stats per category |
| `run_minimal.sh` | ~45 | Run pipeline | Auto-detects Python runner |
...
**Output:**
```
✓ Parsed: 23 entries, 3 colored
  Categories: {
    'System Info': 1, 'Memory': 1, 'Messages': 12, 
    'Checkpoints': 1, 'Actions': 4, 'Error Handling': 1, 'Data Types': 3
  }
```
...
# Actions with nesting
with start_action(action_type="user:login"):
    log.success("Login success", user="alice")
...
### Screenshot (Terminal with ANSI Colors)

> 📸 **Manual screenshot required** - GitHub markdown doesn't render ANSI colors
>
> Run `./run_minimal.sh` and screenshot the output to update this section
