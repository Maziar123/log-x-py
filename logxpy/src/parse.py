...
# Legacy to compact field name mapping for backward compatibility
_LEGACY_TO_COMPACT = {
    "task_uuid": "tid",
    "timestamp": "ts",
    "task_level": "lvl",
...
def _normalize_message_dict(message_dict: dict) -> dict:
    """
    Normalize a message dictionary to use compact field names.

    Supports both legacy Eliot format (task_uuid, timestamp, etc.)
    and new LogXPy compact format (tid, ts, etc.).

    @param message_dict: Dictionary with either legacy or compact field names.
    @return: Dictionary with compact field names.
    """
    if "tid" in message_dict or "ts" in message_dict:
        return message_dict
    normalized = {}
    for key, value in message_dict.items():
        compact_key = _LEGACY_TO_COMPACT.get(key, key)
...
class Task(PClass):
    ...
    def add(self, message_dict):
        ...
        @return: Updated L{Task}.
        """
        # Normalize legacy field names to compact format
        message_dict = _normalize_message_dict(message_dict)
