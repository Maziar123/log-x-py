"""
Command line program for filtering line-based LogXPy logs.
"""
...
class LogFilter:
    """
    Filter LogXPy log lines using a Python expression.
...
USAGE = """\
Usage: cat app.log | python -m logxpy.filter <expr>
...
- Pass through the messages unchanged:
    ...
    $ cat app.log | python -m logxpy.filter J

- Retrieve a specific field from a specific message type, dropping messages
  of other types:

    $ cat app.log | python -m logxpy.filter \\
        "J['field'] if J.get('mt') == 'my:message' else SKIP"
"""
...
def main(sys=sys):
    ...
    if len(sys.argv) != 2:
        ...
        return 1
    LogFilter(sys.argv[1], sys.stdin, sys.stdout).run()
    return 0
