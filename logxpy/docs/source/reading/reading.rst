...
.. code-block:: shell-session
   ...
   $ python examples/stdout.py | logxpy-prettyprint
...
Run ``logxpy-prettyprint --help`` to see the various formatting options; you can for example use a more compact one-message-per-line format.

Additionally, the **highly recommended** third-party `logxpy-tree`_ tool renders JSON-formatted Eliot messages into a tree visualizing the tasks' actions.
...
3. `logxpy-tree`_ has some filtering and searching support built-in.
