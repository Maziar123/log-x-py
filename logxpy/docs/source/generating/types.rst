...
.. code-block:: python
    ...
    from logxpy import Field, MessageType
...
If you pass this function in it will be called with values that are being validated; if it raises ``logxpy.ValidationError`` that value will fail validation.
...
.. code-block:: python
    ...
    from logxpy import MessageType, Field
...
.. code-block:: python
    ...
    from logxpy import MessageType, fields
...
.. code-block:: python
    ...
    from logxpy import MessageType, Field, fields
...
.. code-block:: python
    ...
    from logxpy import ActionType, fields
...
If a message fails to serialize then a ``logxpy:traceback`` message will be logged, along with a ``logxpy:serialization_failure`` message with an attempt at showing the message that failed to serialize.
...
.. code-block:: json
    ...
    {"exception": "exceptions.ValueError",
     ...
     "system": "logxpy:output",
     "reason": "invalid literal for int() with base 10: 'hello'",
     "message_type": "logxpy:traceback"}
    {"timestamp": "2013-11-22T14:16:51.386827Z",
     ...
     "message_type": "logxpy:serialization_failure"}

Testing
-------

The ``logxpy.testing.assertHasAction`` and ``assertHasMessage`` APIs accept ``ActionType`` and ``MessageType`` instances, not just the ``action_type`` and ``message_type`` strings.
