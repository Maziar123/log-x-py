...
``logxpy.logwriter.ThreadedWriter`` is a logging destination that wraps a blocking destination and writes to it in a non-reactor thread.
...
If you want log rotation you can pass in an ``logxpy.FileDestination`` wrapping one of the classes from `twisted.python.logfile`_ as the destination file.
...
.. code-block:: python
   ...
   from logxpy import add_destinations
   from logxpy.twisted import TwistedDestination
...
Trial Integration
-----------------
If you're using Twisted's ``trial`` program to run your tests you can redirect your Eliot logs to Twisted's logs by calling ``logxpy.twisted.redirectLogsForTrial()``.
...
``logxpy.writeFailure`` is the equivalent of ``logxpy.write_traceback``, only for ``Failure`` instances:

.. code-block:: python

    from logxpy import writeFailure
...
Simply substitute ``logxpy.twisted.inline_callbacks`` for ``twisted.internet.defer.inlineCallbacks`` in your code.

To understand why, consider the following example:

.. code-block:: python

     from logxpy import start_action
     from twisted.internet.defer import inlineCallbacks

     @inlineCallbacks  # don't use this in real code, use logxpy.twisted.inline_callbacks
     def go():
...
These problems are solved by using ``logxpy.twisted.inline_callbacks`` instead of ``twisted.internet.defer.inlineCallbacks``.
...
To solve this problem you can use the ``logxpy.twisted.DeferredContext`` class.
...
.. code-block:: python
     ...
     from logxpy import start_action
     from logxpy.twisted import DeferredContext
