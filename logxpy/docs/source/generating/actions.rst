...
If you want to ignore the context and create a top-level task you can use the ``logxpy.start_task`` API.
...
.. code-block:: python
     ...
     from logxpy import start_action, start_task
...
You can explicitly finish an action by calling ``logxpy.Action.finish``.
...
You can get the current ``Action`` by calling ``logxpy.current_action()``.
For example:

.. code-block:: python

   from logxpy import current_action
