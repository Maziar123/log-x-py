...
.. code-block:: shell-session
   ...
   $ pip install logxpy logxpy-tree requests

You can also install it using Conda:

.. code-block:: shell-session

   $ conda install -c conda-forge logxpy logxpy-tree requests

This will install:

1. Eliot itself.
2. `logxpy-tree <https://github.com/jonathanj/eliottree>`_, a tool that lets you visualize Eliot logs easily.
...
.. code-block:: python
   ...
   try:
       check_links(["http://logxpy.readthedocs.io", "http://nosuchurl"])
   except ValueError:
...
1. Tell Eliot to log messages to file called "linkcheck.log" by using ``logxpy.to_file()``.
2. Create two actions using ``logxpy.start_action()``.
   Actions succeed when the ``logxpy.start_action()`` context manager finishes successfully, and fail when an exception is raised.
...
.. code-block:: shell-session
   ...
   $ cat linkcheck.log
   {"action_status": "started", "task_uuid": "b1cb58cf-2c2f-45c0-92b2-838ac00b20cc", "task_level": [1], "timestamp": 1509136967.2066844, "action_type": "check_links", "urls": ["http://logxpy.readthedocs.io", "http://nosuchurl"]}
   ...

So far these logs seem similar to the output of regular logging systems: individual isolated messages.
But unlike those logging systems, Eliot produces logs that can be reconstructed into a tree, for example using the ``logxpy-tree`` utility:

.. code-block:: shell-session
   :emphasize-lines: 3,8,13,16-19,21-23

   $ logxpy-tree linkcheck.log
   b1cb58cf-2c2f-45c0-92b2-838ac00b20cc
   └── check_links/1 ⇒ started
       ├── timestamp: 2017-10-27 20:42:47.206684
       ├── urls: 
       │   ├── 0: http://logxpy.readthedocs.io
       │   └── 1: http://nosuchurl
       ├── download/2/1 ⇒ started
       │   ├── timestamp: 2017-10-27 20:42:47.206933
       │   ├── url: http://logxpy.readthedocs.io
