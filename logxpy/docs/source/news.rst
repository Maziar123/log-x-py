...
* Removed the deprecated ``logxpy.logwriter.ThreadedFileWriter``.
...
* ``Action.continue_task`` now takes ``action_task`` and extra fields to use for the action, so the default ``logxpy:remote_task`` can be changed.
...
* Dask support now includes support for tracing logging of ``dask.persist()``, via wrapper API ``logxpy.dask.persist_with_trace()``.
...
* The ``logxpy-prettyprint`` command line tool now supports a more compact format by using the ``--compact`` argument.
* The ``logxpy-prettyprint`` command line tool now supports outputting in local timezones using the ``--local-timezone`` argument.
...
* ``@logxpy.testing.capture_logging`` now passes ``*args`` and ``**kwargs`` to the wrapped function, as one would expect. Fixes #420. Thanks to Jean-Paul Calderone for the bug report.
...
* ``logxpy.use_asyncio_context()`` is no longer necessary.
  On Python 3.5 and 3.6, however, you should make sure to import ``logxpy`` (or ``aiocontextvars``) before you start your first event loop.
...
* Added lower-level testing infrastructure—``logxpy.testing.swap_logger`` and ``logxpy.testing.check_for_errors``—which is useful for cases when the ``@capture_logging`` decorator is insufficient. For example, test methods that are async, or return Twisted ``Deferred``. See the :doc:`testing documentation<generating/testing>` for details. Thanks to Jean-Paul Calderone for the feature request. Fixes #364.
* ``logxpy.ValidationError``, as raised by e.g. ``capture_logging``, is now part of the public API. Fixed issue #146.

Twisted-related features:

* New decorator, ``@logxpy.twisted.inline_callbacks`` , which is like Twisted's ``inlineCallbacks`` but which also manages the Eliot context. Thanks to Jean-Paul Calderone for the fix. Fixed issue #259.
* ``logxpy.twisted.DeferredContext.addCallbacks`` now supports omitting the errback, for compatibility with Twisted's ``Deferred``. Thanks to Jean-Paul Calderone for the fix. Fixed issue #366.
...
* NumPy integers, floats, bools and arrays are now automatically serialized to JSON, via a new default JSON encoder (``logxpy.json.EliotJSONEncoder``).
* Dask support: replace ``dask.compute()`` with ``logxpy.dask.compute_with_trace()`` to automatically preserve Eliot context for ``Bag`` and ``Delayed`` Dask computations. See :ref:`dask_usage` for details.
* New decorator, ``@logxpy.log_call``, which automatically creates an action that starts when function is called and ends when it returns. See :ref:`log_call decorator`.
...
* ``logxpy.testing.LoggedAction`` has a new method, ``type_tree()``, that returns the tree of action and message types.
  This allows for easier testing of action structure.
* ``logxpy.testing.LoggedAction.of_type`` now accepts the type as a string, not just an ``logxpy.ActionType`` instance.
...
* ``logxpy.add_destinations`` replaces ``logxpy.add_destination``, and accepts multiple Destinations at once.
* ``logxpy.twisted.TwistedDestination`` allows redirecting Eliot logs to ``twisted.logger``.
...
* New APIs: ``logxpy.current_action()`` returns the current ``Action``, and ``Action.task_uuid`` is the task's UUID.
...
* Eliot tasks can now more easily :ref:`span multiple threads <cross thread tasks>` using the new ``logxpy.preserve_context`` API.
* ``logxpy-prettyprint`` command line tool now pretty prints field values in a more informative manner.

Bug fixes:

* ``logxpy-prettyprint`` now handles unparseable lines by skipping formatting them rather than exiting.
...
* ``logxpy-prettyprint`` is a command-line tool that formats JSON Eliot messages into a more human-friendly format.
* ``logxpy.logwriter.ThreadedWriter`` is a Twisted non-blocking wrapper for any blocking destination.
...
* Creating your own ``Logger`` instances is no longer necessary; all relevant APIs now default to using a global one.
  A new testing decorator (``logxpy.testing.capture_logging``) was added to capture global logging.
...
* ``write_traceback`` and ``writeFailure`` no longer require a ``system`` argument, as the combination of traceback and action context should suffice to discover the origin of the problem.
  This is a minor change to output format as the field is also omitted from the resulting ``logxpy:traceback`` messages.
...
* :ref:`logxpy.add_global_fields <add_global_fields>` allows adding fields with specific values to all Eliot messages logged by your program.
...
* Most public methods and functions now have underscore-based equivalents to the camel case versions, e.g. ``logxpy.write_traceback`` and ``logxpy.writeTraceback``, for use in PEP 8 styled programs.
  Twisted-facing APIs and pyunit assertions do not provide these additional APIs, as camel-case is the native idiom.
* ``logxpy.to_file`` outputs log messages to a file.
...
* Added a new API for Twisted ``Deferred`` support: ``logxpy.twisted.DeferredContext``.
* ``logxpy.twisted.redirectLogsForTrial`` will redirect Eliot logs to Twisted's logs when running under the ``trial`` test runner.
