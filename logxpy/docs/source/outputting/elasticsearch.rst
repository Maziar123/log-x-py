...
Assuming each Eliot message is written out as a JSON message on its own line (which is the case for ``logxpy.to_file()`` and ``logxpy.logwriter.ThreadedFileWriter``), the following Logstash configuration will load these log messages into an in-process ElasticSearch database:
