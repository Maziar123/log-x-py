...
Since JSON is a common output format, Eliot provides a utility class that logs to a file, ``logxpy.FileDestination(file=yourfile)``.
Each Eliot message will be encoded in JSON and written on a new line.
As a short hand you can call ``logxpy.to_file``, which will create the destination and then add it automatically.
For example:

.. code-block:: python

    from logxpy import to_file
    to_file(open("logxpy.log", "ab"))
...
.. note::
    ...
    If you're using Twisted you can wrap a ``logxpy.FileDestination`` with a non-blocking :ref:`logxpy.logwriter.ThreadedWriter<ThreadedWriter>`.
...
By default Eliot uses ``logxpy.json.json_default`` to encode objects that the default JSON serializer doesn't handle.
You can customize encoding by passing a custom function to either ``logxpy.FileDestination`` or ``logxpy.to_file``:

.. code-block:: python

   from logxpy.json import json_default
   from logxpy import to_file
   ...
   to_file(open("logxpy.log", "ab"), default=default)
...
Use the ``logxpy.add_global_fields`` API to do so, e.g.:

.. code-block:: python

    import os, sys
    from logxpy import add_global_fields
